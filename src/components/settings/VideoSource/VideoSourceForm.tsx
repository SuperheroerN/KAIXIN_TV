...
import SourceTestButton from './SourceTestButton'
    ...
    >
      <div className="flex items-center justify-between gap-5">
        ...
        <div className="flex w-40 items-center justify-end gap-2">
          <Checkbox checked={isRandomId} onCheckedChange={handleRandomIdChange} disabled={true} />
          <Label className="text-gray-400">使用随机ID</Label>
        </div>
      </div>
    </InputFormItem>
  )
...
// 视频源表单
export default function VideoSourceForm({ sourceInfo, readOnly = false }: { sourceInfo: VideoApi; readOnly?: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onInvalid)}>
      <div className="flex flex-col gap-4 pt-4">
        ...
        {/* 接口测试 */}
        <FieldSet>
          <FieldLegend className="text-xl! font-semibold">接口测试</FieldLegend>
          <FieldDescription>测试该视频源是否支持搜索功能</FieldDescription>
          <FieldGroup>
            <SourceTestButton source={sourceInfo} />
          </FieldGroup>
        </FieldSet>
        <FieldSeparator />
        {/* 高级设置 */}
